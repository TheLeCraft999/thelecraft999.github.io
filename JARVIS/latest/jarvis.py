...
def updater():
    try:
        ...
        if response.status_code == 200:
            ...
            data1 = soup.find('p', id='myData1').get_text()
            data2 = soup.find('p', id='myData2').get_text()
        else:
        ...
        print('--------------------------------------------------------------------------------')
        quit()
    decimal_part1 = int(str(globalvar.VERSION).split('.')[0])
    decimal_part2 = int(str(globalvar.VERSION).split('.')[1]) 
    if int(data1) > int(decimal_part1):
        print(f'{Fore.RED}You are running a very old version! This version will not be supported in the future{Style.RESET_ALL}')
        print(f'Download the latets version: {Fore.LIGHTBLUE_EX}https://thelecraft999.github.io/JARVIS/installer.py {Style.RESET_ALL}')
        print()
        print('--------------------------------------------------------------------------------')
        time.sleep(10)
        quit()  
    elif int(data2) > int(decimal_part2):
        print(f'You are running an old version! Latest: {Fore.RED}{data1}.{data2}{Style.RESET_ALL}')
        print(f'Click here to download a newer Version: {Fore.LIGHTBLUE_EX}https://thelecraft999.github.io/JARVIS/installer.py {Style.RESET_ALL}')
        print()
        print('--------------------------------------------------------------------------------')
    else:
